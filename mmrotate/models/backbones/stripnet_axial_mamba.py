...
class AxialMamba(nn.Module):
    """Axial Mamba gate used between the 3x3 depthwise conv and strip convolutions.

    The module follows the diagram the user provided:
    1) scan along width with one Mamba block,
    2) scan along height with another Mamba block,
    3) sum the two branches,
    4) apply sigmoid gating to the local feature.
    """
    ...
    def __init__(self, dim, d_state=16, d_conv=4, expand=2):
        ...
        if Mamba is None:
            raise ImportError(
                'AxialMamba requires the official mamba-ssm package. '
    ...
    def forward(self, x):
        ...
        row_tokens = self.row_mamba(row_tokens)
        row_feat = row_tokens.reshape(b, h, w, c).permute(0, 3, 1, 2).contiguous()

        col_tokens = x.permute(0, 3, 2, 1).reshape(b * w, h, c)
        col_tokens = self.col_mamba(col_tokens)
        col_feat = col_tokens.reshape(b, w, h, c).permute(0, 3, 2, 1).contiguous()

        gate = torch.sigmoid(row_feat + col_feat)
        return x * gate
